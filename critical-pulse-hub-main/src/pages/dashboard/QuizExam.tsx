...
export default function QuizExam() {
  ...
  const { data: currentData, isLoading: examLoading, refetch: refreshQuestion } = useQuery({
  ...
  if (examLoading || !currentData) {
    return <div className="min-h-screen flex items-center justify-center font-mono text-sm text-ink-faint">Initializing Exam Environment...</div>;
  }

  const { question, attempt } = currentData;
    ...
    <div className="flex flex-col min-h-screen bg-chalk-warm">
      ...
      <div className="h-[64px] bg-chalk border-b border-border-soft px-6 lg:px-8 flex items-center justify-between sticky top-0 z-40">
        <div className="flex flex-col">
            ...
            <span className="font-sans font-extrabold text-sm text-slate truncate max-w-[200px]">{currentData.attempt.exam_title || 'Assigned Test'}</span>
        </div>
      ...
      <div className="h-[72px] bg-chalk border-t border-border-soft px-6 lg:px-8 flex items-center justify-between sticky bottom-0 z-40">
        ...
        <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="group flex items-center gap-2 bg-slate text-chalk rounded-sm px-10 py-3 font-sans text-sm font-bold hover:bg-slate-light transition-all shadow-lg active:scale-95 cursor-pointer"
            >
              {currentIdx === total ? 'Finish Exam' : <>Next Question <ChevronRight size={16} /></>}
            </button>
        </div>
      </div>
    </div>
